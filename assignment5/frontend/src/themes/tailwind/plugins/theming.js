...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--tiger-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--tiger-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--tiger-${name}-rgb))`;
    };
...
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--tiger-${e(key)}`, value], [`--tiger-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`tiger.customProps.background.${colorScheme}`);
            const foreground = theme(`tiger.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--tiger-${e(key)}`, value], [`--tiger-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--tiger-${e(key)}`, value], [`--tiger-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
